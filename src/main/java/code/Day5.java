...
public class Day5 {
    ...
    public static void main(String[] args) {
        ...
        String humidityToLocationInput = """
        ...
        long[][] waterToLightArray = create2DArray(waterToLightInput);
    ...
    private static long getMapping(long[][] arr , long input){
        for (int i = 0; i < arr.length; i++) {

            if(arr[i][1] <= input && input <= arr[i][1]+arr[i][2]-1){
                long x = arr[i][1]+arr[i][2]-1-input;
                long y = arr[i][0]+arr[i][2] -1;
                return y-x;
            }
        }
        return input;
    ...
    private static long[][] create2DArray(String input) {
        String[] lines = input.split("\n");
        long[][] array = new long[lines.length][3];

        for (int i = 0; i < lines.length; i++) {
            String[] values = lines[i].split(" ");
            for (int j = 0; j < 3; j++) {
                array[i][j] = Long.parseLong(values[j]);
            }
        }

...
public class Day1 {
    ...
    public static void main(String[] args) {
        String input = """
                ...
                """;
        System.out.println(extractTotalCalibrationValue(input));
    }

    private static long extractTotalCalibrationValue(String input) {
        String[] words = input.split("\n");
        long sum = 0L;
        for(String  word : words){
            System.out.print(word + " ");
            int val = extractCalibrationValue(word);
            System.out.println(val);
            sum += val;
        }
        return sum;
    }
    private static int extractCalibrationValue(String word) {
        String[] spelledWords = {"one","two","three","four","five","six","seven","eight","nine"};
        ...
        for(int i=0;i<word.length();i++){
            ...
            //For part two
                ...
                for (int j = 0; j < spelledWords.length; j++) {
                    if (word.substring(i).startsWith(spelledWords[j]))
                        list.add(j + 1);
                }
            }
        }
        ...
        if(list.isEmpty())
            ...
            if(list.size() ==1 ){
                return list.get(0)*10 + list.get(0);
            }
            else{
                return list.get(0)*10 + list.get(list.size()-1);
            }
        }

...
public class Day19 {
    ...
    public static void main(String[] args) {
        ...
        Map<String, List<String>> map = createMapFromInput(input.split("\n\n")[0]);
        List<String> parts = convertToList(input.split("\n\n")[1]);
        part1(map,parts);
    }

    private static void part1(Map<String, List<String>> map ,List<String> parts){
        long total = 0;
        for (String p : parts) {
            if (accept(p, map)) {
                int sum = Arrays.stream(p.split(","))
    ...
    private static boolean accept(String s,Map<String, List<String>> map){
        String[] parts = s.split(",");
        String wf = "in";
        while (true) {
            List<String> rules = map.get(wf);
            for (String cond : rules) {
                // If A , then stop processing this part
                if (cond.equals("A")) {
                    return true;
                }
                // If R , then stop processing this part
                else if (cond.equals("R")) {
                    return false;
                } else if (cond.contains(":")) {
                    String expr = cond.split(":")[0];
                    char ch = expr.charAt(0);
                    boolean found = false;
                    for (String part : parts) {
                        char ch2 = part.charAt(0);
                        if (ch == ch2) {
                            char op = expr.charAt(1);
                            int op1 = Integer.parseInt(part.substring(2));
                            int op2 = Integer.parseInt(expr.substring(2));
                            if ((op == '>' && evaluateGreater(op1, op2)) || (op == '<' && evaluateLess(op1, op2))) {
                                found = true;
                                wf = cond.split(":")[1];
                                // If A , then stop processing this part
                                if (wf.equals("A")) {
    ...
    public static boolean evaluateGreater(int op1, int op2) {
        return op1 > op2;
    }

    public static boolean evaluateLess(int op1, int op2) {
        return op1 < op2;
    }



    public static Map<String, List<String>> createMapFromInput(String input) {
        ...
        for (String line : lines) {
            ...
            String key = parts[0];
            String valueStr = parts[1].substring(0, parts[1].length() - 1); // Remove the trailing '}'
    ...
    public static List<String> convertToList(String input) {
        ...
        for (String line : lines) {
            ...
            String element = line.substring(1, line.length() - 1);

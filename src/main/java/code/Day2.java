...
public class Day2 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(numberOfGames(input));
        System.out.println(numberOfGames2(input));
    }

    private static int numberOfGames(String input){
        String[] lines = input.split("\n");
        Pattern pattern = Pattern.compile("Game (\\d+):");
        int count =0;
        for (String line : lines){
            Matcher matcher = pattern.matcher(line);
            if(matcher.find() && isGamePossible(line)){
                System.out.println("Game ID " + matcher.group(1));
    ...
    private static boolean isGamePossible(String line) {
        // Define the pattern for extracting red, green, and blue balls
        Pattern pattern = Pattern.compile("(\\d+)\\s*(red|green|blue)");
        ...
        for (String game : games) {
            ...
            // Find and check red, green, and blue balls in each game set
            while (matcher.find()) {
                int count = Integer.parseInt(matcher.group(1));
                String color = matcher.group(2);
                System.out.println(count + " " + color + " ball(s)");
    ...
    private static int numberOfGames2(String input){
        String[] lines = input.split("\n");
        int sum =0;
        for (String line : lines){
                int power = powerGame(line);
                sum += power;
        }
        return sum;
    }

    private static int powerGame(String line) {
        // Define the pattern for extracting red, green, and blue balls
        Pattern pattern = Pattern.compile("(\\d+)\\s*(red|green|blue)");
        // Split input into games
        String[] games = line.split(";");
        ...
        for (String game : games) {
            ...
            // Create a matcher
            Matcher matcher = pattern.matcher(game);
            // Find and print red, green, and blue balls in each game set
            while (matcher.find()) {
                int count = Integer.parseInt(matcher.group(1));
                String color = matcher.group(2);
                System.out.println(count + " " + color + " ball(s)");
